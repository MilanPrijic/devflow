...
import {CollectionBaseSchema, PaginatedSearchParamsSchema} from "@/lib/validaitons";
import handleError from "@/lib/handlers/errors";
import {DTOCollection, DTOQuestion} from "@/database";
...
export async function toggleSaveQuestion(params: CollectionBaseParams): Promise<ActionResponse<{ saved: boolean }>> {
    ...
    try {
        ...
        const collection = await DTOCollection.findOne({
            question: questionId,
            author: userId
        });

        if(collection) {
            await DTOCollection.findByIdAndDelete(collection._id);
...
export async function getSavedQuestions(params: PaginatedSearchParams): Promise<ActionResponse<{ collection: Collection[], isNext: boolean }>> {
    ...
    try {
        ...
        const [totalCount] = await DTOCollection.aggregate([
        ...
        const questions = await DTOCollection.aggregate(pipeline);
